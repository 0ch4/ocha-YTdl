...
var jsc = (function (meriyah, astring) {
  ...
  function getFromPrepared(code) {
    ...
    let fn;
    const tt = (typeof globalThis !== 'undefined' && globalThis.trustedTypes) || (typeof window !== 'undefined' && window.trustedTypes);
    if (tt && tt.createPolicy) {
      const globalObj = typeof window !== 'undefined' ? window : (typeof self !== 'undefined' ? self : globalThis);
      if (!globalObj.__ytSolverPolicy) {
        try {
          globalObj.__ytSolverPolicy = tt.createPolicy('yt-solver-eval', {
            createScript: (s) => s
          });
        } catch (e) {
          // If policy creation fails (e.g. name taken), we will fallback
        }
      }
      const policy = globalObj.__ytSolverPolicy;
      const rawScript = '(function(_result){\n' + code + '\n})';
      const trustedScript = policy ? policy.createScript(rawScript) : rawScript;
      fn = globalObj.eval(trustedScript);
    } else {
      fn = Function('_result', code);
    }
    fn(resultObj);
    return resultObj;
  }
