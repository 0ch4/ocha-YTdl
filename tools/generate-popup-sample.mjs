...
<svg xmlns="http://www.w3.org/2000/svg" width="360" height="920" viewBox="0 0 360 920" role="img" aria-label="ocha-YTdl popup sample">
  ...
  <g font-family="-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif">
    <g transform="translate(14 12)">
      ...
      <text x="332" y="13" fill="#555" font-size="8.5" text-anchor="end">v${esc(data.version)}</text>
    </g>
    ...
    <g transform="translate(14 238)">
      ...
      <rect x="220" y="10" width="112" height="26" rx="4" fill="#1a1a1a" stroke="#333"/>
      <text x="8" y="27" fill="#bcbcbc" font-size="9">解像度すべて</text>
      <text x="118" y="27" fill="#bcbcbc" font-size="9">FPSすべて</text>
      <text x="228" y="27" fill="#bcbcbc" font-size="9">拡張子すべて</text>
      ...
      <rect x="113" y="226" width="106" height="28" rx="4" fill="#cc0000"/>
      <rect x="226" y="226" width="106" height="28" rx="4" fill="#cc0000"/>
      <text x="53" y="244" fill="#fff" font-size="9" font-weight="700" text-anchor="middle">映像DL</text>
      <text x="166" y="244" fill="#fff" font-size="9" font-weight="700" text-anchor="middle">音声DL</text>
      <text x="279" y="244" fill="#fff" font-size="9" font-weight="700" text-anchor="middle">両方DL</text>

      <rect x="0" y="266" width="332" height="32" rx="8" fill="#1565c0"/>
      <text x="166" y="286" fill="#fff" font-size="10.5" font-weight="700" text-anchor="middle">映像+音声を合成して保存</text>

      <text x="0" y="318" fill="#4caf50" font-size="9.5">${esc(data.note)}</text>
      <text x="0" y="338" fill="#777" font-size="8.5">${esc(data.formatSummary)}</text>
...
function renderWrappedTitle(text, x, y, fontSize, maxLines) {
  ...
  if (trimmed.length === 0) trimmed.push(String(text || 'video'));
  return trimmed.map((line, index) => {
    const dy = index === 0 ? 0 : fontSize + 4;
    return `<text x="${x}" y="${y + dy}" fill="#f2f2f2" font-size="${fontSize}" font-weight="700">${escapeXml(line)}</text>`;
  }).join('');
}
