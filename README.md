...
![切り出しパネル](docs/images/clip-panel.png)
...
![ダウンロードウィンドウ](docs/images/download-window.png)

このウィンドウはページから独立しているので、**ダウンロード中に別の動画へ移動しても中断しません**。

## ショート動画

ショートでも同じことができます。右側のボタンの列（高評価・共有・リミックス）の一番下に **切り出し** が並びます。

![ショート動画の切り出しパネル](docs/images/shorts.png)

押すと、動画の右の余白にパネルが開きます。範囲の指定も画質の選択も、通常の動画と同じです。

ウィンドウを狭めると、YouTube はボタンの列を動画の上に重ねてきます。

![狭いときのショート動画](docs/images/shorts-narrow.png)

パネルを開く余白が無くなるので、このときは **切り出し** を押すとその場で最高画質のまま保存します（範囲の指定はできません）。進捗はボタンのラベルに出ます。
