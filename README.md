...
- `0:05` から `0:10` までのような任意の時間範囲を指定して切り出し保存
...
時間指定を入力した場合は、対象ファイルを取得したあと `sandbox/muxer.html` 上の ffmpeg.wasm で `-c copy` による切り出しを行います。再エンコードしないため高速ですが、動画によってはキーフレーム位置の都合で開始位置が少し前後することがあります。
